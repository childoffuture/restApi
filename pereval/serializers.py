...
class PerevalSerializer(serializers.Serializer):
    """
    Класс-сериализатор. Преобразует принятый JSON в модель Pereval.
    ...
    def create(self, validated_data):
        """
        Создание объекта Pereval. Создание происходит по следующему правилу:
    ...
    def update(self, instance, validated_data):
        """
        Редактирование объекта Pereval по следующему правилу:
        ...
        :return: обновленный объект Pereval
        """
        ...
        # Обновление путем создания новой записи Pereval с ключом instance.id, тк возникла проблема обновления raw_data:
        # при вызове instance.raw_data = raw_data
        # вместо нового словаря почему то в бд сохранялся список из словаря и пустого поля
        pereval = Pereval(id=instance.id, date_added=validated_data.pop('add_time', instance.date_added), raw_data=raw_data,
                          images=validated_data.pop('images'))
        pereval.save()
        return instance
